...
export const connectToRedisCache = async () => {
    try {
        ...
        client = redis.createClient({
            ...
            password: process.env.CACHE_PWD && process.env.CACHE_PWD !== "null" ? process.env.CACHE_PWD : undefined,
        });
        ...
        client.on("connect", function () {
            ...
            logger.info(`Connected to the cache server @${process.env.CACHE_HOSTNAME}:${cacheConfig.port}`);
        });
    ...
    if (readReplicaConfig) {
        try {
            clientReadReplica = redis.createClient({
                ...
                password:
                    process.env.CACHE_READ_REPLICA_PWD && process.env.CACHE_READ_REPLICA_PWD !== "null"
