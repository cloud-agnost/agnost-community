...
export const updateDatabaseHandler = (connection, queue) => {
            ...
            async function (msg) {
                ...
                if (["Deploying", "Redeploying"].includes(envStatus)) {
                    ...
                    if (now - millisecondsFromEpoch >= config.get("general.maxMessageWaitMinues") * 60 * 1000) {
                                    ...
                                    logs: [
                                        {
                                            ...
                                            status: "Error",
                                            message: "Database update timed out due to errors",
                                        },
                                    ],
                                    type: "db",
                                },
                        ...
                        logger.info(
                            t(
                                "Database update timed out for app '%s' version '%s' to environment '%s'",
                                msgObj.app.name,
                ...
                logger.info(
                    t(
                        "Started updating app '%s' version '%s' database(s) '%s'",
                        msgObj.app.name,
                        msgObj.env.version.name,
                ...
                if (result.success) {
                    ...
                    logger.info(
                        t(
                            "Completed updating app '%s' version '%s' databases '%s' successfully",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.databases.map((entry) => entry.name).join(", ")
                        )
                    ...
                    logger.error(
                        t(
                            "Cannot update app '%s' version '%s' database(s) '%s'",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.databases.map((entry) => entry.name).join(", ")
                        ),
