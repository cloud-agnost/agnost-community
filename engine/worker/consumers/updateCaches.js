...
export const updateCachesHandler = (connection, queue) => {
            ...
            async function (msg) {
                ...
                if (["Deploying", "Redeploying"].includes(envStatus)) {
                    ...
                    if (now - millisecondsFromEpoch >= config.get("general.maxMessageWaitMinues") * 60 * 1000) {
                                    ...
                                    logs: [
                                        {
                                            ...
                                            status: "Error",
                                            message: "App version cache update timed out due to errors",
                                        },
                                    ],
                                    type: "db",
                                },
                        ...
                        logger.info(
                            t(
                                "Cache update timed out for app '%s' version '%s' to environment '%s'",
                                msgObj.app.name,
                ...
                logger.info(
                    t(
                        "Started updating app '%s' version '%s' caches '%s'",
                        msgObj.app.name,
                        msgObj.env.version.name,
                        msgObj.caches.map((entry) => entry.name).join(", ")
                    )
                );

                // Create the deployment manager and deploy the version
                let manager = new DeploymentManager(msgObj);
                let result = await manager.updateCaches();

                if (result.success) {
                    channel.ack(msg);
                    logger.info(
                        t(
                            "Completed updating app '%s' version '%s' caches '%s' successfully",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.caches.map((entry) => entry.name).join(", ")
                        )
                    ...
                    logger.error(
                        t(
                            "Cannot update app '%s' version '%s' caches '%s'",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.caches.map((entry) => entry.name).join(", ")
                        ),
