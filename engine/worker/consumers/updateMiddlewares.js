...
export const updateMiddlewaresHandler = (connection, queue) => {
            ...
            async function (msg) {
                ...
                if (["Deploying", "Redeploying"].includes(envStatus)) {
                    ...
                    if (now - millisecondsFromEpoch >= config.get("general.maxMessageWaitMinues") * 60 * 1000) {
                                    ...
                                    logs: [
                                        {
                                            ...
                                            status: "Error",
                                            message: "Middlewares update timed out due to errors",
                                        },
                                    ],
                                    type: "db",
                                },
                        ...
                        logger.info(
                            t(
                                "Middlewares update timed out for app '%s' version '%s' to environment '%s'",
                                msgObj.app.name,
                ...
                logger.info(
                    t(
                        "Started updating app '%s' version '%s' middlewares '%s'",
                        msgObj.app.name,
                        msgObj.env.version.name,
                        msgObj.middlewares.map((entry) => entry.name).join(", ")
                    )
                );

                // Create the deployment manager and deploy the version
                let manager = new DeploymentManager(msgObj);
                let result = await manager.updateMiddlewares();

                if (result.success) {
                    channel.ack(msg);
                    logger.info(
                        t(
                            "Completed updating app '%s' version '%s' middlewares '%s' successfully",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.middlewares.map((entry) => entry.name).join(", ")
                        )
                    ...
                    logger.error(
                        t(
                            "Cannot update app '%s' version '%s' middlewares '%s'",
                            msgObj.app.name,
                            msgObj.env.version.name,
                            msgObj.middlewares.map((entry) => entry.name).join(", ")
                        ),
