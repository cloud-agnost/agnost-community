...
export const undeployVersionHandler = (connection, queue) => {
	connection.createChannel(function (error, channel) {
		...
		channel.consume(
			...
			async function (msg) {
				...
				if (
					...
					if (
						...
						sendMessage(msgObj.env._id, {
							...
							description: t(
								"App version undeployment timed out due to errors"
							),
							timestamp: Date.now(),
							data: msgObj,
							identifiers: {
						...
						logger.info(
							t(
								"Undeployment timed out for app '%s' version '%s' from environment '%s'",
								msgObj.app.name,
				...
				logger.info(
					t(
						"Started undeploying app '%s' version '%s' from environment '%s'",
						msgObj.app.name,
						msgObj.env.version.name,
						msgObj.env.name
					)
				...
				let result = await manager.undeployVersion();
				console.log("***result", result);
				if (result.success) {
					channel.ack(msg);
					logger.info(
						t(
							"Completed undeploying app '%s' version '%s' from environment '%s' successfully",
							msgObj.app.name,
							msgObj.env.version.name,
							msgObj.env.name
						)
					...
					logger.error(
						t(
							"Cannot undeploy app '%s' version '%s' from environment '%s'",
							msgObj.app.name,
							msgObj.env.version.name,
							msgObj.env.name
						),
