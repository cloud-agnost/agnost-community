...
export class MongoDBManager extends DBManager {
	...
	async createDatabase() {
		...
		conn.db(`${this.getEnvId()}_${this.getDbId()}`);
	...
	async dropDatabase() {
		const conn = await this.getConn();
		await conn.db(`${this.getEnvId()}_${this.getDbId()}`).dropDatabase();
	}
	...
	async getAppDB() {
		if (!this.appDB) {
			const conn = await this.getConn();
			this.appDB = conn.db(`${this.getEnvId()}_${this.getDbId()}`);
		}
	...
	collectionHasModel(collection) {
		...
		for (let i = 0; i < models.length; i++) {
			...
			if (collection.collectionName === model.name && model.type === "model")
				return true;
		}
			...
			if (model.type === "model") {
				...
				if (this.modelHasCollection(collections, model) === false) {
					...
					if (prevModel && prevModel.name !== model.name) {
						...
						this.addLog(
							t("Renamed collection '%s' to '%s'", prevModel.name, model.name)
						);
					}
				}
			}
	...
	async processFieldNameChanges() {
			...
			for (let j = 0; j < model.fields.length; j++) {
				...
				if (prevModel && prevField) {
					if (field.name !== prevField.name) {
						...
						if (!fieldChanges[collectionName])
							fieldChanges[collectionName] = [];
		...
		for (let key in fieldChanges) {
			...
			for (let i = 0; i < renameFields.length; i++) {
				...
				this.addLog(
					t(
						"Renamed field '%s' to '%s' in model '%s",
						item.oldName,
						item.newName,
						item.modelPath
					)
	...
	async dropDeletedFields() {
		...
		for (let key in fieldDeletes) {
			...
			for (let i = 0; i < deleteFields.length; i++) {
				...
				this.addLog(
					t("Deleted field '%s' in model '%s", item.fieldName, item.modelPath)
				);
			}
		}
	}
	...
	async ensureIndices() {
		...
		for (let i = 0; i < collections.length; i++) {
			...
			this.addLog(
				t(
					"Completed processing collection index defitions for model '%s'",
					model.name
				)
			);
		}
	...
	getRequiredIndices(model, indices = null, textIndexDef = null) {
		...
		for (let i = 0; i < model.fields.length; i++) {
			...
			if (this.isIndexRequired(field)) {
				...
				if (
					indices.length + 1 >
					config.get("general.maxMongoDBIndexNumberPerCollection")
				)
					continue;
