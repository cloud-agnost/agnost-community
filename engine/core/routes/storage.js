...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // Split the file name into parts by the last dot
      let newFilePath = null;
      const dotIndex = filePath.lastIndexOf(".");
      if (dotIndex === -1) {
        // No dot found, append " (1)" to the filename
        newFilePath = `${fileName} (${files.length})`;
      } else {
        const namePart = filePath.substring(0, dotIndex);
        const extensionPart = filePath.substring(dotIndex); // Include the dot
        // Generate the new file name by adding " (1)" before the extension
        newFilePath = `${namePart} (${files.length})${extensionPart}`;
      }
