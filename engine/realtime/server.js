...
async function initExpress(i18n) {
	...
	const realtimeServer = setUpRealtimeServer(server);

	// Initialize rate limiter
	rateLimitManager.initialize(getRedisClient());
	// Set up garbage collector to manage memory consumption of the realtime server
	setUpGC();

	return { expressServer: server, realtimeServer };
}

function handleProcessExit(expressServer, realtimeServer) {
