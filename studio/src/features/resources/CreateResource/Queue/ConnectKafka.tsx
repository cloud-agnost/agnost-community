...
import ConnectKafkaWithSASL from './ConnectKafkaWithSASL';
import ConnectKafkaWithSSL from './ConnectKafkaWithSSL';
...
export default function ConnectKafka() {
	...
	useEffect(() => {
		form.setValue('access.format', 'simple');
	}, []);
	return (
		<>
			<CreateResourceItem title={t('resources.queue.connFormat')}>
				<FormField
					control={form.control}
					name='access.format'
					render={({ field }) => (
			...
			<CreateResourceItem title={t('resources.queue.brokers')}>
				{fields.map((f, index) => (
					<div className='flex gap-4' key={f.id}>
						<FormField
							control={form.control}
							name={`access.brokers.${index}`}
							render={({ field }) => (
			...
			{form.watch('access.format') === 'ssl' && <ConnectKafkaWithSSL />}
			{form.watch('access.format') === 'sasl' && <ConnectKafkaWithSASL />}
		</>
	);
}
