...
router.post(
	...
	async (req, res) => {
		try {
			const { version } = req;
			const { type, buckets, start, end } = req.query;
			...
			const parsedFilter = JSON.parse(JSON.stringify(filter), dateReviver);
			const pipeline = [
				{
					$match: {
						...parsedFilter,
...
router.post(
	...
	async (req, res) => {
		try {
			const { version, body } = req;
			const { page, size, type, sortBy, sortDir } = req.query;
