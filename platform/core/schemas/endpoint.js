					...
					.custom((value) => {
						...
						if (!regex.test(value)) {
							throw new AgnostError(
								t(
									"Endpoint names can include only numbers, letters and underscore (_) characters"
								)
							);
						}
						...
						if (regex2.test(value)) {
							throw new AgnostError(
								t("Endpoint names cannot start with a number")
							);
						}

						if (value.startsWith("_")) {
							throw new AgnostError(
								t("Endpoint names cannot start with underscore (_) character")
							);
						}
