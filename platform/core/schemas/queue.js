			...
			return [
				body("name")
					...
					.custom(async (value, { req }) => {
						...
						queues.forEach((queue) => {
							if (
								(queue.name.toLowerCase() === value.toLowerCase() &&
									type === "create") ||
								(queue.name.toLowerCase() === value.toLowerCase() &&
									type === "update" &&
									req.queue._id.toString() !== queue._id.toString())
							)
								throw new AgnostError(
									t("Queue with the provided name already exists")
								);
						});
				...
				body("resourceId")
					...
					.custom(async (value, { req }) => {
						...
						// Check if the selected resource is a queue
						if (resource.type !== "queue")
							throw new AgnostError(
								t(
									"The selected resource '%s' (%s) is not a queue",
									resource.name,
									resource.instance
								)
							);
						...
						if (resource.status !== "OK")
							throw new AgnostError(
								t(
									"Only resorces in ready (OK) status can be mapped to a queue. The selected '%s' resoure '%s' is in '%s' status",
									resource.instance,
									resource.name,
									resource.status
								)
